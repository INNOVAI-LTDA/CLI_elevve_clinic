ALTER TABLE deva_elevveclinic_users
  ADD COLUMN IF NOT EXISTS organization_id BIGINT;

UPDATE deva_elevveclinic_users AS u
SET organization_id = o.id
FROM deva_elevveclinic_organizations AS o
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'fk_deva_elevveclinic_users_organization_id'
  ) THEN
    ALTER TABLE deva_elevveclinic_users
      ADD CONSTRAINT fk_deva_elevveclinic_users_organization_id
...
FROM deva_elevveclinic_organizations AS o
LEFT JOIN deva_elevveclinic_users AS u
