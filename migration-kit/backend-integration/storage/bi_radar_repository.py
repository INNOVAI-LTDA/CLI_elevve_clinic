...
class BiRadarRepository:
    ...
    def list_responses(self, patient_id: str) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
    ...
    def list_patients(self) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT id, patient_code, name, program_name "
                "FROM deva_elevveclinic_bi_patients "
                "ORDER BY id"
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def get_patient(self, patient_id: str) -> dict[str, Any] | None:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT id, patient_code, name, program_name, status "
                "FROM deva_elevveclinic_bi_patients "
                "WHERE id = ?",
                (patient_id,),
            ).fetchone()
    ...
    def get_snapshots(
        ...
        with self._connect() as conn:
            if pillar_code is not None:
                rows = conn.execute(
                    "SELECT id, patient_id, calculated_at, pillar_code, "
                    ...
                    "WHERE patient_id = ? AND pillar_code = ? "
                    "ORDER BY calculated_at DESC",
                    (patient_id, pillar_code),
                ).fetchall()
            else:
                rows = conn.execute(
                    "SELECT id, patient_id, calculated_at, pillar_code, "
    ...
    def insert_snapshot(self, row: dict[str, Any]) -> None:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                "(id, patient_id, calculated_at, pillar_code, raw_score, "
